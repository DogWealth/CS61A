...
def pingpong(n):
    ...
    def helper(index, value, step):
        ...
        return value
    return helper(1, 1, 1)
#基于上面这个程序，我们也可以改写斐波那契数列来缩短运行时间
"""def Fibonacci(n):
    def helper(index, one, sec, val):
        if n == 0:
            return 0
        elif n == 1:
            return 1
            ...
            one, sec = sec, val
            return helper(index+1, one, sec, one + sec)
        return val
...
def missing_digits(n):
    ...
    "*** YOUR CODE HERE ***"
    if n >= 10:
        rest, process = n // 10, n % 100
        num = process % 10 - process // 10 - 1
        num = num if num >= 0 else 0
        return missing_digits(rest) + num
    else:
        return 0
    ...
    def helper(change, coin):
        if change == 0:
            ...
            return helper(change - coin, coin)\
                    + helper(change, descending_coin(coin))
