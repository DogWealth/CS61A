...
def balanced(m):
    ...
    '''
    #base case test1
    if is_planet(left(m)) and is_planet(right(m)):
        if total_weight(left(m)) == total_weight(right(m)):
            return True
        else:
            return False
    '''
    #base case2，直接让planet算平衡就行
    if is_planet(m):
        return True
    else:
        l_m = end(left(m))
        ...
        if balanced(l_m) and balanced(r_m):
            if total_weight(l_m) * l_len == total_weight(r_m) * r_len:
                return True
            else:
                return False
        else:
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    if is_leaf(t):
        ...
        lst = []
        for b in branches(t):
            lst += [replace_loki_at_leaf(b, lokis_replacement)]#注意要加[]
...
def has_path(t, word):
    ...
    if is_leaf(t) or len(word) == 1:
        return True if label(t) == word else False
    elif label(t) != word[0]:
        return False
    else:
        lst = []
        for b in branches(t):
            lst += [has_path(b, word[1:])]
        return any(lst)
...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    minus_y = interval(-1 * upper_bound(y), -1 * lower_bound(y))
