    ...
    def vend(self):
        if self.amount == 0:
            ...
            if self.balance < self.price:
                s = f'You must add ${self.price - self.balance} more funds.'
                ...
                self.amount -= 1
                s = f'Here is your {self.product} and ${self.balance - self.price} change.'
                self.balance = 0
        return s
    
    def add_funds(self, funds):
        if self.amount == 0:
            s = f'Nothing left to vend. Please restock. Here is your ${funds}.'
        else:
            self.balance += funds
            s = f'Current balance: ${self.balance}'
        return s
...
def two_list(vals, amounts):
    ...
    for _ in range(amounts[0]-1):
        tmp = Link(vals[0], tmp)
    tmp_pointer.rest = two_list(vals[1:], amounts[1:])
    return tmp
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            ...
            next_fib = VirFib(self.value + self.pre_value)
...
def is_bst(t):
    ...
    def bst_max(t):
        if t.is_leaf():
            ...
            min_label = t.label
            for b in t.branches:
                min_label = max(min_label, bst_min(b))
            return min_label
    ...
    if len(t.branches) == 1:
        if is_bst(t.branches[0]):
            return True
    elif is_bst(t.branches[0]) and is_bst(t.branches[1]):
