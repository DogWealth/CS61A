...
def closer_city(lat, lon, city_a, city_b):
    ...
    "*** YOUR CODE HERE ***"
    c = make_city("", lat, lon)
    dis_a = distance(c, city_a)
    return get_name(city_a) if dis_a < distance(c, city_b) else get_name(city_b)
...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        return tree(label(t), [[leaf] for leaf in leaves])
    else:
        lst = []
        for b in branches(t):
            lst += [sprout_leaves(b, leaves)]
