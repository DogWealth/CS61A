...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not tmp.is_hive:
            if tmp.bees and (self.min_range <= distance <= self.max_range):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        bees = [bee for bee in bees if bee.place]
        if self.place == None:
            for bee in bees:
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        #看后面的GameState类里面，动作执行者是place.ant，但是container携带的ants才是动作执行者
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while place:
            if place.ant:
                place.ant.buff()#修改了Ant类里面的buff()方法
            place = place.exit
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scare_turns:#停止运动的时候，恐惧不会减少回合，直接跳过
            self.scare_turns -= 1
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2):
        ...
        """
        self.has_queen = False#problem12
        self.time = 0
        self.food = food
