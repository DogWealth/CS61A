...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def find_topics(ps):
        word_list = split(lower(remove_punctuation(ps)))
...
def accuracy(typed, reference):
    ...
    for i in range(word_len):
        try:
            ...
            break
    return 100 * count / word_len if word_len != 0 else 0.0
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for valid_word in valid_words:
        if typed_word == valid_word:
            ...
            diff_score = diff_function(typed_word, valid_word, limit)
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if start == goal: #minimize the amount of computation
        return 0
    elif limit == 0:  #minimize the amount of computation
        return 1
    elif len(start) == 0 or len(goal) == 0:
        return len(start) + len(goal)
    else:
        change_val = int(start[0]!=goal[0])
        return change_val + feline_flips(start[1:], goal[1:], limit-change_val)
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == goal:  # Fill in the condition
        ...
        return len(start) + len(goal) #这相当于直接add补长度
    elif start[0] == goal[0]:
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def num_cor_words(ws1, ws2):
        for i in range(len(ws1)):
            if ws1[i] != ws2[i]:
                return i
        return len(ws1)
    progress = num_cor_words(sofar, prompt) / len(prompt)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    players_index = range(len(get_times(match)))
    words_index = range(len(get_words(match)))
    players_list = [[] for i in players_index]
    for i in words_index:
        fastest_player = min(players_index, key=lambda x: time(match, x, i))#高阶函数，players_index实际上作为函数key的参数x
        players_list[fastest_player].append(word_at(match, i))
    return players_list
