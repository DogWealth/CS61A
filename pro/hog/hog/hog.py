...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    #debug了很久，主要原因是忽略了每次调用dice()都会改变返回值
    outcomes = 0
    outcome = 0
    for i in range(num_rolls):
        cur_num = dice()
        if cur_num == 1: #dice() == 1:,错误，又一次的调用了dice()改变了返回值
            outcome = 1
        else: 
            outcomes += cur_num#outcomes += dice(),这是错误的，再一次调用了dice()
    if outcome: 
        return outcome
    return outcomes
    # END PROBLEM 1
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    else: return picky_piggy(opponent_score)
    # END PROBLEM 3
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        last = 0
        if not who:
            record_gain = score0 - last_score
            last = score0
        else:
            record_gain = score1 - last_score
            last = score1

        if record_gain > running_high:
            print("{} point(s)! That's a record gain for Player {}!".format(record_gain, who))
        return announce_highest(who, last, max(record_gain, running_high))
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    def result(*args):
        ...
        for i in range(trials_count):
            v += original_function(*args)
        return v / trials_count
    return result
    # END PROBLEM 8
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if picky_piggy(opponent_score) + score == opponent_score or picky_piggy(opponent_score) >= cutoff: return 0
    return num_rolls
    # END PROBLEM 11
